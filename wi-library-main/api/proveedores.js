...
passport.authenticate("jwt", { session: false }),
body("proveedor.nombre").isString().isLength({min: 1, max: 45}),
body("proveedor.direccion").isString().isLength({min: 1, max: 45}),
body("proveedor.telefono").isInt().isLength({min: 1, max: 999999999999999}),
body("proveedor.mail").isEmail().isLength({min: 1, max: 45}),
async (req,res) =>{
    const validacion = validationResult(req)
    if (!validacion.isEmpty()) {
        res.status(400).send({errors: validacion.errors})
    ...
    const [rows] = await db.execute(
      ...
      { nombre: proveedor.nombre, direccion: proveedor.direccion, telefono: proveedor.telefono, 
        mail: proveedor.mail, }
    );
